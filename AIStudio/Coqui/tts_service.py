...
class AudioRequest(BaseModel):
    ...
    output_path: Optional[str] = None  # 完整的输出路径，优先级高于output_filename
...
async def generate_audio(request: AudioRequest):
    ...
    try:
        # 生成输出文件路径 - 优先使用完整路径，否则使用默认目录
        if request.output_path:
            output_path = request.output_path
            logger.info(f"Using custom output path: {output_path}")
        elif request.output_filename:
            output_path = f"/home/n8n/AIStudio/jobs/audio/{request.output_filename}"
            logger.info(f"Using default directory with filename: {output_path}")
        else:
            output_path = f"/home/n8n/AIStudio/jobs/audio/{task_id}.mp3"
            logger.info(f"Using generated filename: {output_path}")
